...
class TestRecordFullInteraction:
    def test_full_lifecycle_records_and_exports(self, app_ctx) -> None:
        ...
        active = recording_list(ctx=ctx)
        assert active["active_count"] >= 1

        # End recording
        ended = recording_end(recording_id=rid, ctx=ctx)
        ...
        verification_result = synpareia.verify_export(
            proof["proof"], public_keys={profile.id: profile.public_key}
        )
    ...
    def test_cannot_add_after_end(self, app_ctx) -> None:
        ctx, _ = app_ctx
        start = recording_start(description="short", ctx=ctx)
        rid = start["recording_id"]
        recording_append(recording_id=rid, content="msg", ctx=ctx)
        recording_end(recording_id=rid, ctx=ctx)

        # Adding after end should return an error, not silently succeed
        result = recording_append(recording_id=rid, content="late", ctx=ctx)
        assert "error" in result

    def test_tampered_proof_fails_verification(self, app_ctx) -> None:
        ctx, _ = app_ctx
        start = recording_start(description="tamper test", ctx=ctx)
        rid = start["recording_id"]
        for i in range(2):
            recording_append(recording_id=rid, content=f"m{i}", ctx=ctx)
        recording_end(recording_id=rid, ctx=ctx)
        proof = recording_proof(recording_id=rid, ctx=ctx)["proof"]
    ...
    def test_recording_list_lists_ended_recording(self, app_ctx) -> None:
        ...
        ctx, _ = app_ctx
        start = recording_start(description="lifecycle", ctx=ctx)
        rid = start["recording_id"]
        recording_append(recording_id=rid, content="m", ctx=ctx)
        recording_end(recording_id=rid, ctx=ctx)

        # Whatever the current contract is, recording_proof must succeed
        # after recording_end — that's the verifier's only view.
        proof = recording_proof(recording_id=rid, ctx=ctx)
        assert "error" not in proof
    ...
    def test_proof_bundle_is_self_contained(self, app_ctx) -> None:
        ...
        ctx, _ = app_ctx
        start = recording_start(description="self-contained", ctx=ctx)
        rid = start["recording_id"]
        recording_append(recording_id=rid, content="m1", ctx=ctx)
        recording_append(recording_id=rid, content="m2", ctx=ctx)
        recording_end(recording_id=rid, ctx=ctx)
        proof = recording_proof(recording_id=rid, ctx=ctx)
